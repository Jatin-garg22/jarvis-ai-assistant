...
ALLOWED_PLUGINS = {
    'weather', 'news', 'search', 'gmail', 'dictionary', 'timer', 'clock',
    'website', 'qr', 'wifi', 'tasks', 'battery', 'location', 'open',
    'play', 'hear'
}
...
class PluginManager(object):
    ...
    def _validate_plugins(self, plugins):
        def partition(plugins):
            ...
            for plugin_to_validate in plugins:
                ...
                if not self._is_plugin_allowed(plugin_to_validate):
                    plugins_incompatible.append(
                        (plugin_to_validate.get_name(),
                         'blocked by command allowlist'))
                    continue
    ...
    def _is_plugin_allowed(self, plugin_to_validate):
        return plugin_to_validate.get_name() in ALLOWED_PLUGINS
